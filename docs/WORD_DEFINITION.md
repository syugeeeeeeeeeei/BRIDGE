...
BRIDGEでは、アーキテクチャ上の責務境界を表す「メインコンポーネント」と、メインコンポーネント内部の実装責務を分割する「サブコンポーネント」を区別する。

#### 2.7.1 Main Component

- **Term ID:** `main-component`
- **日本語正式名:** メインコンポーネント
- **分類:** アーキテクチャ
- **関連語:** Component Rule
- **非推奨の同義表現:** 主要コンポーネント、上位コンポーネント、親コンポーネント

BRIDGE全体において独立した責務境界、依存規則、禁止事項および不変条件を持つ、橋梁由来の大文字名で識別されるコンポーネント。

現行のメインコンポーネントは、`GATE`、`TRUSS`、`ANCHOR`、`BOLTS`、`BEARING`、`ULTRASOUND`、`TRAFFIC`、`HEALTHY`である。`CORE`は共有基盤として同等に規則管理されるが、アルゴリズムまたは製品責務を持つメインコンポーネントとは区別する。

メインコンポーネントの新設には、既存コンポーネントへ自然に所属しない独立責務、固有の不変条件、安定した依存境界および`COMPONENT_RULE.md`が必要である。ファイル数、コード量または実装技術だけを理由に新設してはならない。

#### 2.7.2 Subcomponent

- **Term ID:** `subcomponent`
- **日本語正式名:** サブコンポーネント
- **分類:** アーキテクチャ
- **関連語:** Main Component
- **非推奨の同義表現:** 下位コンポーネント、子コンポーネント、内部コンポーネント

一つのメインコンポーネントまたは製品境界の内部で、その親が所有する責務を実装上の変更理由ごとに分割した単位。Package、Directoryまたは責務別File群として表現できる。

サブコンポーネントは、親の`COMPONENT_RULE.md`、依存規則および不変条件に従う。独自の橋梁由来メインコンポーネント名を持たず、親の責務を拡張または横断してはならない。

例として、TRAFFICのScenario、Generator、Runner、Metrics、Artifact Builder、HEALTHYのProfile、Validation、Oracle、Comparison、Evaluationはサブコンポーネントである。CLIやHTTP ServerのCommand、Handler、Middleware、Policyも製品境界内のサブコンポーネントであり、BRIDGEのメインコンポーネントではない。
...
文脈上メインコンポーネントとサブコンポーネントの双方を含み得る一般語。規範、依存関係または所有者を厳密に示す箇所では、`Main Component`または`Subcomponent`を明示する。
...
| `hypothesis` | Hypothesisの生成、分岐、凍結、再開または枝刈りを行う |
| `schedule` | Task登録、選択、状態遷移またはepoch制御を行う |
| `handoff` | Handoff Requestの生成、局所実行または結果適用を行う |
| `evidence` | Evidenceの検証、登録、統合または失効処理を行う |
...
全Graph Familyで幾何学的な対角点を意味するわけではないため、旧既定endpoint名は使用しない。
...
本計測前に実行し、Summary Statistics、acceptance判定およびraw測定Runから除外する事前実行。

v0.15.0以降、Warm-upではObservation、Collector、Trace保存を無効化し、破棄される高頻度eventを生成しない。
...
- **計測境界:** 公開API呼出し直前から結果返却まで。TRAFFICによるGraph Generationは含めない。
...
| 旧既定endpoint名 | Generatorにより意味が異なる | `generator_default_endpoints` |
...
TRUSSが、runnable taskの選択、Work grant、実行、結果回収、Evidence統合および終了判定を一まとまりとして行う決定論的な調停単位。

Epochは単なるループ回数ではない。共有状態は原則としてEpoch境界で固定順に統合する。
...
Node集合として表現される、Hypothesis、EvidenceまたはHandoffの適用範囲。包含、重複、拡張を判定できなければならない。
...
探索Sessionの継続に必要な接続点または局所状態の参照。Handoffの入力または出力として使用する。

SnapshotはSession全体の再開状態であり、Checkpointは局所的な接続・再利用点である。

### 16.4 Handoff

- **Term ID:** `handoff`
- **分類:** TRUSS・ANCHOR・BOLTS

ANCHORの探索状態を破棄せず、局所目的をBOLTSへ依頼し、その結果をANCHORへ適用して再開するプロトコル。

全体問題をゼロから別Solverへ渡す旧Fallbackとは異なる。

### 16.5 Handoff Purpose

- **Term ID:** `handoff-purpose`
- **分類:** BOLTS Capability

Handoffが要求する局所能力。標準値は次のとおり。
...
Solver間で共有できる事実または推定。ID、生成Solver、Hypothesis ID、Scope、Generated Work、ProofClass、Value、失効条件を持つ。

値だけをEvidenceとして扱ってはならない。
...
Evidenceの証明強度と意味を示す列挙。

| 値 | 意味 |
|---|---|
| `empirical` | 経験的推定。証明値として使用不可 |
| `admissible_lower_bound` | 最適Costを超えないことが保証されたLower Bound |
| `unreachable` | Scope内の到達不能証明 |
| `exact` | Candidateの最適性証明 |

Proof Classの変更は型変換ではなく証明生成である。経験的値を上位Classへ昇格してはならない。

### 16.8 Termination Status

- **Term ID:** `termination-status`
- **分類:** 実行結果
- **推奨フィールド:** `termination_status`

一回のRoute実行が終了した排他的理由。
...
複数booleanを組み合わせて終了理由を推定してはならない。

### 16.9 State Reuse Ratio

- **Term ID:** `state-reuse-ratio`
- **分類:** 再利用指標
- **推奨フィールド:** `state_reuse_ratio`

後続TaskまたはSolverが実際に再利用した有効状態量を、再利用可能状態量または対象処理量で正規化した比率。分母をScenarioまたはschemaで明示する。

未計測時に0を設定してはならない。0は計測した結果、再利用がなかった場合だけに使用する。

### 16.10 Duplicate Work Ratio

- **Term ID:** `duplicate-work-ratio`
- **分類:** 効率指標
- **推奨フィールド:** `duplicate_work_ratio`

同一Scope、同一状態または同等の探索事実に対して重複して消費したWorkの割合。

### 16.11 Timing Valid

- **Term ID:** `timing-valid`
- **分類:** Timing検証
- **推奨フィールド:** `timing_valid`

Timing値が測定契約を満たし、性能比較へ使用可能であることを示す判定。

`timing_valid=true`でsolver時間が0の場合は契約違反である。表示上のend-to-end時間が非ゼロでも、内部Timingが無効なら正式比較へ使用しない。

### 16.12 Reachability Proof

- **Term ID:** `reachability-proof`
- **分類:** 到達可能性・証明

SourceとTargetが接続される、または完全探索により接続されないことを確定した証明。

重み付きPathの最小Costを証明するものではない。Reachability SolverがPathを返しても`optimality_proven`は自動的にtrueにならない。

### 16.13 Benchmark Invariant

- **Term ID:** `benchmark-invariant`
- **分類:** TRAFFIC・研究データ品質

Raw Runを有効な研究データとして受理するために必ず満たす整合条件。違反時は補正や推定を行わず、Runをfail-closedで拒否する。


## Benchmark Artifact Terms

- **Benchmark Execution**: Scenarioを一度展開して実行した単位。
- **Execution Directory**: 展開済み成果物を保存するディレクトリ。
- **Benchmark Artifact**: 実行条件、結果、環境、観測、検査結果として永続化される単一成果物。
- **Artifact Bundle**: 1回のBenchmark Executionに属する成果物集合。
- **Benchmark Archive**: Artifact Bundleを格納したZIP。
- **Run ID**: Runの正式な一意識別子。
- **Run Ordinal**: シャッフル後を含む実際の実行順。1始まり。
- **Run Directory**: `run-%06d`形式の保存名。意味情報を持つ識別子ではない。
- **Graph Snapshot**: Runで実際に使用したGraphとQueryの完全保存。
- **Canonical Artifact**: 分析の正本となる機械可読成果物。
- **Derived Artifact**: 正本から再生成可能な集計・表示用成果物。
...
実行過程に含まれる処理を、実運用上の責務とBRIDGEの公開interface境界に基づいて分類した区間。Phaseは処理責務、計測境界および性能指標の意味を明確にするための分類であり、単なる進捗状態ではない。

処理の実装コンポーネント、関数または内部的な実行順序だけでPhaseを決定してはならない。
...
BRIDGEへ渡す入力を外部で準備するPhase。Scenario解釈、Graph生成、Query生成および実行条件確定を含む。
...
BRIDGEが入力を受理してから、経路結果および要求されたTrace結果を外部利用可能な形式で返却するまでのPhase。内部Graph構築、Graph分析、特徴抽出、経路探索および結果構築を含む。
...
BRIDGE返却後の結果を外部で集計、分析、評価、保存または可視化するPhase。外部SimulatorによるTrace可視化を含む。
