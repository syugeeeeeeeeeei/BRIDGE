...
def _local_repair(G: Graph, source: Node, target: Node, path: Sequence[Node], cfg: DG6Config, budget: int) -> PathResult:
...
def _grid_detour_strategy(G: Graph, source: Node, target: Node, cfg: DG6Config) -> tuple[Optional[PathResult], Dict[str, Any]]:
...
def _budget(cfg: DG6Config, G: Graph) -> int:
    return max(8, int(math.ceil(len(G.adj) * cfg.target_work_ratio)))


def _quality_budget_available(cfg: DG6Config, G: Graph, spent: int, found: bool) -> int:
    if not found:
        return 0
    budget = _budget(cfg, G)
...
def _geometric_corridor_strategy(G: Graph, source: Node, target: Node, cfg: DG6Config) -> tuple[Optional[PathResult], Dict[str, Any]]:
    budget = _budget(cfg, G)
...
def _portal_strategy(G: Graph, source: Node, target: Node, cfg: DG6Config) -> tuple[Optional[PathResult], Dict[str, Any]]:
    budget = _budget(cfg, G)
    candidates: List[PathResult] = []
    spent = 0
...
def _hub_aware_strategy(G: Graph, source: Node, target: Node, cfg: DG6Config) -> tuple[Optional[PathResult], Dict[str, Any]]:
    budget = _budget(cfg, G)
    degrees = {u: len(G.adj.get(u, [])) for u in G.adj}
...
def _weighted_cost_strategy(G: Graph, source: Node, target: Node, cfg: DG6Config) -> tuple[Optional[PathResult], Dict[str, Any]]:
    budget = _budget(cfg, G)
    candidates: List[PathResult] = []
    spent = 0
    first_found_work = None
...
def mrpc_dg6(G: Graph, source: Node, target: Node, **kwargs: Any) -> PathResult:
    """MRPC-DG6 reference implementation.

    DG6 targets a bounded-quality best-route policy rather than exact shortest paths:
    use structure-specific candidate generation, local exact connectors, and local repair;
    never compute a source-target exact path for oracle replacement unless fallback_exact=True.
    """
    cfg = DG6Config(**{k: v for k, v in kwargs.items() if k in DG6Config.__annotations__})
    start = time.perf_counter()
    outer_mem = _memory_begin() if cfg.measure_memory else True
    pre_work = 0
    if cfg.enable_component_precheck:
        reachable, pre_work = _component_reachable(G, source, target)
        if not reachable:
            peak = (_memory_end(outer_mem) if cfg.measure_memory else 0.0)
            return PathResult([], math.inf, False, True, "mrpc_dg6", work_expanded_nodes=pre_work, parallel_steps=1, time_ms=(time.perf_counter()-start)*1000, peak_memory_kib=peak, telemetry={"variant":"mrpc_dg6", "strategy":"component_precheck", "component_reachable": False, "within_target_ratio_estimated": True, "target_work": _budget(cfg, G)})
    else:
        if source not in G.adj or target not in G.adj:
            peak = (_memory_end(outer_mem) if cfg.measure_memory else 0.0)
            return PathResult([], math.inf, False, False, "mrpc_dg6", work_expanded_nodes=0, parallel_steps=1, time_ms=(time.perf_counter()-start)*1000, peak_memory_kib=peak, telemetry={"variant":"mrpc_dg6", "strategy":"missing_endpoint", "component_reachable": False, "target_work": _budget(cfg, G)})

    features = _graph_features(G, source, target)
    strategy = str(kwargs.get("strategy", "auto"))
    if strategy == "auto":
        strategy = _choose_strategy(features)
    ...
    if strategy not in strategy_fns:
        strategy = "geometric_corridor" if G.pos else "hub_aware"

    # Algorithmic grid cut specialization.  A full barrier column is a cheap,
    # non-oracle unreachable proof for grid-like maps; avoid generic emergency routing.
    grid_like, grid_barriers, grid_hard_cut = _grid_barrier_columns(G, source, target)
    if grid_hard_cut:
        peak = (_memory_end(outer_mem) if cfg.measure_memory else 0.0)
        elapsed = (time.perf_counter() - start) * 1000
        return PathResult([], math.inf, False, True, "mrpc_dg6", work_expanded_nodes=0, parallel_steps=1, time_ms=elapsed, peak_memory_kib=peak, telemetry={"variant":"mrpc_dg6", "strategy":"grid_cut_precheck", "component_reachable": False, "grid_hard_cut": True, "oracle_used": False, "exact_source_target_oracle_used": False, "target_work": _budget(cfg, G), "query_work_units": 0, "total_work_including_preprocessing": 0})
    grid_result, grid_tel = _grid_detour_strategy(G, source, target, cfg)
    if strategy == "geometric_corridor" and grid_result is not None and grid_result.found and grid_tel.get("grid_direct_gap_path"):
        result, st = grid_result, grid_tel
        strategy = "grid_detour"
    else:
        result, st = strategy_fns[strategy](G, source, target, cfg)
    fallback_used = False
    # If a chosen strategy cannot produce any route, try one alternate approximate strategy before optional exact fallback.
    if result is None or not result.found:
        alt_order = ["portal", "geometric_corridor", "weighted_cost", "hub_aware"]
        for alt in alt_order:
            if alt == strategy or alt not in strategy_fns:
                continue
            result, alt_tel = strategy_fns[alt](G, source, target, cfg)
            st = {**st, "primary_strategy_failed": True, "alternate_strategy": alt, **{f"alt_{k}": v for k, v in alt_tel.items()}}
            if result and result.found:
                strategy = alt
                break
    # If all budgeted strategies failed, first check reachability.  This prevents
    # disconnected graphs from wasting time in generic emergency routing.
    reachability_check_work = 0
    if result is None or not result.found:
        reachable, reachability_check_work = _component_reachable(G, source, target)
        if not reachable:
            result = PathResult([], math.inf, False, True, "dg6_reachability_no_path", work_expanded_nodes=reachability_check_work, parallel_steps=1, telemetry={"component_reachable": False})
            st = {**st, "component_reachable": False, "reachability_check_work": reachability_check_work, "strategy_work_units": int(st.get("strategy_work_units", 0)) + reachability_check_work}
    emergency_used = False
    emergency_work = 0
    if result is None or not result.found:
        # Soft-budget safety: the N/2 target is the normal planning budget.
        # If no valid path is found, DG6 may spend an explicit emergency
        # approximate budget to preserve route availability.  This is not an
        # exact fallback and is reported as budget overrun.
        emergency_used = True
        cap = max(_budget(cfg, G), int(len(G.adj) * 1.10))
        if G.pos:
            # Different detour shapes prefer different aggressiveness. Try a
            # conservative beam first, then a more target-biased beam if needed.
            result = None
            for hw in (1.20, 2.40):
                r = _beam_astar_path(G, source, target, max_expansions=cap, heuristic_weight=hw, solver_name=f"dg6_emergency_beam_{hw:.2f}")
                emergency_work += r.work_expanded_nodes
                if r.found:
                    result = r
                    break
                result = r
        else:
            result = _budgeted_bidirectional_dijkstra(G, source, target, max_expansions=cap, solver_name="dg6_emergency_bidir")
            emergency_work = result.work_expanded_nodes if result else 0
    if (result is None or not result.found) and cfg.fallback_exact:
        fallback_used = True
        result = dijkstra(G, source, target, solver_name="dg6_exact_fallback")
    ...
    # DG6 production work target is node-touch/expansion based. Edge relaxations are
    # still returned separately, but telemetry total_work follows the N/2 target.
    total_relax = pre_work + (result.work_relaxations if result else 0)
    total_expanded = pre_work + int(st.get("strategy_work_units", 0)) + int(emergency_work)
    peak = (_memory_end(outer_mem) if cfg.measure_memory else 0.0)
    elapsed = (time.perf_counter() - start) * 1000
    tel = {
        "variant": "mrpc_dg6",
        ...
        "emergency_path_used": emergency_used,
        "emergency_work": emergency_work,
        "oracle_used": False,
        "exact_source_target_oracle_used": False,
        "valid_path": valid,
        "path_distance": pd,
        "precheck_work": pre_work,
        "target_work": _budget(cfg, G),
        "query_work_units": total_expanded,
        "total_work_including_preprocessing": total_expanded,
        "raw_relaxations": total_relax,
        "work_goal_ratio": total_expanded / max(1, len(G.adj)),
        "features": features,
        **st,
    }
    return PathResult(path, dist, found, False, "mrpc_dg6", total_relax, total_expanded, result.queue_pushes if result else 0, result.queue_pops if result else 0, max(3, (result.parallel_steps if result else 0) // 3 + 3), elapsed, peak, tel)
