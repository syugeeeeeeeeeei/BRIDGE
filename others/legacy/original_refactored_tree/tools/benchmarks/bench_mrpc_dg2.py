...
from bridge_py.solvers.mrpc_cg import mrpc_directional_backtrack_v2, mrpc_directed_greedy
...
def main():
    ...
    for n in NODE_SIZES:
      for trial in range(1,TRIALS+1):
        ...
        # previous baseline with exact check disabled impossible, so keep target previous from old public function but this includes internal check; shown as old reference only.
        solvers += [
          ('mrpc_dg2_w1', mrpc_directional_backtrack_v2(G,s,t,workers=1,backtrack_width=3,budget_ratio=0.1)),
          ('mrpc_dg2_w4', mrpc_directional_backtrack_v2(G,s,t,workers=4,backtrack_width=3,budget_ratio=0.1)),
          ('mrpc_dg2_w8', mrpc_directional_backtrack_v2(G,s,t,workers=8,backtrack_width=3,budget_ratio=0.1)),
          ('mrpc_dg2_w4_wide', mrpc_directional_backtrack_v2(G,s,t,workers=4,backtrack_width=6,budget_ratio=0.1)),
        ]
        for name,res in solvers: raw.append(row(n,trial,name,res,exd,G.edge_count()))
        print('done',n,trial,flush=True)
    raw_path=OUT/'mrpc_dg2_bench_raw.csv'
    ...
    sum_path=OUT/'mrpc_dg2_bench_summary.csv'
    ...
    for n in NODE_SIZES:
      ...
      for solver in ['dijkstra','bidirectional_dijkstra','mrpc_dg2_w1','mrpc_dg2_w4','mrpc_dg2_w8','mrpc_dg2_w4_wide']:
        s=by[(n,solver)]
    ...
    rat_path=OUT/'mrpc_dg2_bench_ratios.csv'
