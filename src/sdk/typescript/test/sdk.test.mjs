...
test("bundled route", async () => { const c = await BridgeClient.local(); const r = await c.route(request); assert.equal(r.result.path_found, true); assert.equal(r.result.path_cost, 1); });
