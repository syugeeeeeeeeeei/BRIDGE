...
func benchmarkGrid() *core.AdjacencyGraph {
	...
	for y := 0; y < side; y++ {
		for x := 0; x < side; x++ {
			id := core.NodeID(y*side + x)
			g.SetPosition(id, core.Point{X: float64(x), Y: float64(y)})
			if x+1 < side {
				_ = g.AddEdge(id, id+1, 1)
			}
			if y+1 < side {
				_ = g.AddEdge(id, id+side, 1)
			}
		}
	}
	return g
...
func benchSolver(b *testing.B, s Solver) {
	g := benchmarkGrid()
	req := core.RouteRequest{Source: 0, Target: 1023, Workers: 1}
	o := bearing.NullObserver{}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		r := s.Solve(context.Background(), g, req, core.WorkBudget{}, o)
		if !r.Found {
			b.Fatal("not found")
		}
	}
...
func BenchmarkWeightedAStarRef(b *testing.B) {
	benchSolver(b, WeightedAStar{ID: "weighted_astar", Weight: 1.12})
}
